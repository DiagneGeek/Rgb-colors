...
const validCssPropValue = (value, name, el, config) => {
  if (value.includes('[') && config && config.variables) {
    let variables = config.variables
    for (let v of Object.keys(variables)) {
      let old = value
      value = replaceAll(value, `[${v}]`, variables[v])
      if(old !== value) storage.reactives.push({el, name, reactive:v})
    }
    if (value.includes('[')) {
      return err(`${value.slice(value.indexOf('[')+1, value.indexOf(']')).trim()} is not defined on config.variables`)
    }
  }
  ...
  return {
    ...
    'row-r': 'row-reverse',
    'sm': config?.sm || '0.875rem',
    'lg': config?.lg || '1.125rem',
    'xl': config?.xl || '1.25rem',
  } [value] || value
}


const useDStyle = (el, input, config = null) => {
  let styles = input.split(' ')
  let output = {}
  for (let style of styles) {
    let isSpecial = false;
    [isSpecial, output] = useSpecials(style, output, config,el)
    ...
    if (name == '$') {
      if (!config.snippets[value]) return err(`the Snippet "${value}" is not defined.`);
      output = {
        ...output,
        ...useDStyle(el, config.snippets[value], config)
      }
      continue
    }
    ...
    value = validCssPropValue(value, name, el, config)
...
const update = (name, newVal) => {
   const Vs = storage.config.variables
   if(Vs == null) err("the prop 'variables' on config is not found!")
   if(Vs[name] == undefined) err(name+" is not found on variables")
   Vs[name] = newVal
   const reactive = storage.reactives.find(r => r.reactive == name)
   if(!reactive) return
   newVal = newVal.split(" ").join("_")
   reactive.el.style[reactive.name] = newVal
  }
